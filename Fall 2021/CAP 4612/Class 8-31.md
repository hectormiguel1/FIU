...
| | Accuracy \(\frac {\sum True Positive + \sum True Negative} {\sum Total Population}\) = MATH | Positive Predictive Value (PPV), Precision = MATH | False Ommision Rate (FOR) = MATH | Positive Likelyhood ratio  = (LR+) = TPR/FPR | Diagnostics Odds Ratio (DOR) = LR+/LR- | 
| | | False Discovery Rate (FDR) = MATH  | Negative Predictive Value = (NPV) = MATH | Nagive Likelyhood ratio = (LR-) = FNR/TNR | |